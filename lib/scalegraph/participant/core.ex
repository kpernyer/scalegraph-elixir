defmodule Scalegraph.Participant.Core do
    ...
    unless role in Schema.participant_roles() do
      ...
      result = :mnesia.transaction(fn ->
        case :mnesia.read(Schema.participants_table(), id) do
          [] ->
            record = {Schema.participants_table(), id, name, role, created_at, metadata}
            :mnesia.write(record)
            {:ok, %{
              id: id,
              name: name,
  ...
  def list_participants(role \\ nil) do
    result = :mnesia.transaction(fn ->
      :mnesia.foldl(
        fn {_table, id, name, r, created_at, metadata}, acc ->
          if role == nil or r == role do
            [%{id: id, name: name, role: r, created_at: created_at, metadata: metadata} | acc]
          else
            acc
          end
        end,
  ...
  """
  def create_participant_account(participant_id, account_type, initial_balance \\ 0, metadata \\ %{})
      ...
      case :mnesia.read(Schema.participants_table(), participant_id) do
        ...
        [_participant] ->
          ...
          case :mnesia.read(Schema.accounts_table(), account_id) do
            [] ->
              record = {Schema.accounts_table(), account_id, participant_id, account_type, initial_balance, created_at, metadata}
              :mnesia.write(record)
              {:ok, %{
                id: account_id,
                participant_id: participant_id,
